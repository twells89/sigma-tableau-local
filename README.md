...
| Window / table calcs (`RUNNING_SUM`, `RUNNING_AVG/MIN/MAX`, `WINDOW_SUM/AVG/MIN/MAX/COUNT`, `LOOKUP`, `PREVIOUS_VALUE`, `RANK`, `RANK_DENSE`, `RANK_UNIQUE`, `INDEX`, `FIRST`, `LAST`) | `kind:sql` helper element with explicit Snowflake `OVER()` clauses (Sigma DM has no working partitioned/ordered window formulas). Partition keys come from worksheet `rows` shelves; order keys from time-truncated `cols` shelves. The `DATE_TRUNC` grain is derived from the cols-shelf prefix (`yr:`→year, `qr:`→quarter, `mn:`→month, `wk:`→week, `dy:`/`md:`→day; no time prefix → no DATE_TRUNC wrapper). Explicit "Compute Using" addressing (`<table-calculation>` with `<address>` children, plus Table Across/Down direction) overrides the rows/cols heuristic. Multiple window calcs sharing the same partition+order share a single helper. |
...
- **Window calc Compute Using — Pane / Cell** — "Specific Dimensions" addressing and Table (Across/Down) direction are parsed from `<table-calculation>` in the TWB. Pane and Cell scopes (which require pane-bounds awareness) are not yet supported; the converter falls back to the rows/cols heuristic and emits a warning. Edit the helper SQL post-import if needed.
